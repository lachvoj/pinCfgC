...
// loopable IF
void Switch_vLoop_withFb(LOOPRE_IF_T *psHandle, uint64_t u64ms);
void Switch_vLoop_noFb(LOOPRE_IF_T *psHandle, uint64_t u64ms);
    ...
    uint8_t u8FbPin)
{
    ...
    psHandle->u64ImpulseStarted = 0U;
    psHandle->u64ImpulseDuration = 300U;
    if (u64ImpulseDuration != 0U)
        psHandle->u64ImpulseDuration = u64ImpulseDuration;
...
void Switch_vLoop(SWITCH_HANDLE_T *psHandle, uint64_t u64ms)
{
    ...
    {
        if (psHandle->u64ImpulseStarted == 0U)
        {
            psHandle->u64ImpulseStarted = u64ms;
            Switch_vWritePin(psHandle, (uint8_t) true);
        }
        else if ((u64ms - psHandle->u64ImpulseStarted) >= psHandle->u64ImpulseDuration)
        {
            psHandle->u64ImpulseStarted = 0U;
            psPresentHnd->bStateChanged = false;
